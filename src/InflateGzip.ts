...
/**
 * A processor for decompressing Gzip data.
 */
class InflateGzip {
    ...
    public decode(data: Uint8Array): Uint8Array {
        let windowBits = this.windowBits ?? 31;
        if (windowBits < 0) windowBits = -windowBits;
        while (windowBits < 16) windowBits = windowBits + 16;
        while (windowBits > 31) windowBits = windowBits - 16;
